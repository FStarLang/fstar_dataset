...
def get_argument():
    ...
    parser.add_argument("--solution_key", "-k", type=str, default="generated_response/responses")
...
def main():
    args = get_argument()
    assert args.dataset_dir is not None and os.path.exists(args.dataset_dir), "Dataset directory not found."
    global DATSET_DIR
    ...
    for gf in generated_files: 
        with open(gf, "r") as fp:
            ...
            fp.close()
    tasks = [(t, args.check_ground_truth, args.solution_key, args.timeout) for t in tasks]
    pool = Pool(args.num_workers) if args.num_workers is not None and args.num_workers > 1 else None
    ...
    for example, res, truths, metrics in bar:
        detailed_results.append({"example": example, "results": res, "truths": truths, "metrics": metrics })
    ...
    if args.output_dir is not None:
        ...
        input_file_name = os.path.basename(args.input_files[0]) if len(args.input_files) == 1 else args.output
        os.makedirs(args.output_dir, exist_ok=True)
        with open(os.path.join(args.output_dir, input_file_name.replace(".json", "_detailed_results.json")), "w") as fp:
            json.dump(detailed_results, fp, indent=4)
            fp.close()
        with open(os.path.join(args.output_dir, input_file_name.replace(".json", "_aggregate_metrics.json")), "w") as fp:
